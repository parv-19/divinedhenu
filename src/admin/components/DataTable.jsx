  ...
  return (
    <div className="overflow-hidden rounded-lg border border-ritual-border bg-ritual-card">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-ritual-border text-sm">
          <thead className="bg-ritual-background/70">
            <tr>
              {columns.map((column) => (
                <th key={column.key} className="px-4 py-3 text-left font-semibold text-ritual-muted">
                  {column.label}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-ritual-border">
            {rows.length ? rows.map((row) => (
              <tr key={row._id || row.id} className="align-middle">
                {columns.map((column) => (
                  <td key={column.key} className="px-4 py-3 text-ritual-text">
